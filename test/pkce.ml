...
let create_challenge () =
  let verifier = verifier_of_string code_verifier in
  let challenge = make_code_challenge verifier in
  let (challenge_string, _meth) = challenge_to_code_challange_and_method challenge in
  check_string
...
let verify_challenge () =
  let verifier = verifier_of_string code_verifier in
  let challenge = challenge_of_string ~transformation:`S256 code_challenge in
  Alcotest.(check bool)
...
let suite, _ =
  Junit_alcotest.run_and_report ~package:"oidc" "pkce"
    [("OIDC - pkce", tests)]
